...
def processMap(img):
    ...
    mask = cv2.inRange(img,np.array([50,50,50]),np.array([255,255,255]))
    ...
    _,dt = cv2.threshold(dt,0.07*dt.max(),255,0)
    dt = np.uint8(dt)
...
def interpolate(nodePaths):
    interpolatedPaths = []
    for path in gPaths:
        interpolatedPath = [path[0]]
        for i in range(1,len(path)):
            pair = [path[i-1],path[i]]
            nodePath = getCurPath(pair,nodePaths)
            interpolatedPath.extend(nodePath[1:])
        i = len(interpolatedPath)-1
        while (i != 0):
            d=dist2([interpolatedPath[i][0]-interpolatedPath[i-1][0],interpolatedPath[i][1]-interpolatedPath[i-1][1]])
            if d<625:
                _=interpolatedPath.pop(i)
            i-=1
        interpolatedPaths.append(interpolatedPath)
